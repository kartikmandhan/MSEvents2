  ...
  $.StackSlider.prototype = {
    _init: function (options) {
      ...
      if (this.support) {
        this.transEndEventName =
          transEndEventNames[Modernizr.prefixed("transition")] + ".stackslider";
    ...
    _layout: function () {
      ...
      if (this.itemsCount > 1) {
        this.$navigation = $(
          "<nav><span>Previous</span><span>Next</span></nav>"
        );
      ...
      $items.each(function () {
        var $this = $(this);
        html +=
          '<div class="st-item" data-title="' +
          $this.children("div.st-title").html() +
          '">' +
          $this.children("div.st-item").html() +
          "</div>";
      });
    ...
    _navigate: function (dir, flow) {
      var self = this,
        classes = "st-left st-center st-right st-leftflow st-rightflow",
        dirclass,
        posclass,
        pileOut,
        pileIn,
      ...
      setTimeout(function () {
        if (
          (flow &&
            (self.current === 0 || self.current === self.itemsCount - 1)) ||
          !flow
        ) {
    ...
    _initItems: function () {
      var self = this,
        ...
        $currentItem = this.$items
          ...
          .addClass("st-center")
          .show(),
        pileHFactor = this.options.piles
          ? Math.max(this.$lPile.height(), this.$rPile.height()) / 2
          : 0;
        ...
        if (self.support) {
          $item.css({
            transition:
              ...
              self.options.easing,
            transformOrigin:
              "50% " + (self.radius + itemH / 2 - pileHFactor) + "px",
          });
        }
  ...
  $.fn.stackslider = function (options) {
    ...
    if (typeof options === "string") {
      ...
      this.each(function () {
        if (!instance) {
          logError(
            "cannot call methods on stackslider prior to initialization; " +
